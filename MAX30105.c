/***************************************************
  ...
  Modified by Naveen Kumar to port into AVR IOT WA Development Board
...
bool MAX30105_begin() {
    ...
    uint8_t part_id = MAX30105_readPartID();
    //printf("part_id=%04x\n", part_id);
    if (part_id != MAX_30105_EXPECTEDPARTID) {
...
void MAX30105_bitMask(uint8_t reg, uint8_t mask, uint8_t thing) {
    ...
    uint8_t originalContents = i2c_read1ByteRegister(MAX30105_ADDRESS, reg);

    // Zero-out the portions of the register we're interested in
    originalContents = originalContents & mask;

    // Change contents
    i2c_write1ByteRegister(MAX30105_ADDRESS, reg, originalContents | thing);
}
...
uint8_t MAX30105_getINT1(void) {
    return (i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_INTSTAT1));
}

uint8_t MAX30105_getINT2(void) {
    return (i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_INTSTAT2));
}
...
void MAX30105_softReset(void) {
    ...
    // Timeout after 100ms
    //uint32_t startTime = TIME_getCurrent();
    uint32_t startTime = millis();
    //while (TIME_getDiffTime(TIME_getCurrent(), startTime)*1000 < 100) {
    while ((millis() - startTime) < 100) {
        uint8_t response = i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_MODECONFIG);
...
void MAX30105_setPulseAmplitudeRed(uint8_t amplitude) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_LED1_PULSEAMP, amplitude);
}

void MAX30105_setPulseAmplitudeIR(uint8_t amplitude) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_LED2_PULSEAMP, amplitude);
}

void MAX30105_setPulseAmplitudeGreen(uint8_t amplitude) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_LED3_PULSEAMP, amplitude);
}

void MAX30105_setPulseAmplitudeProximity(uint8_t amplitude) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_LED_PROX_AMP, amplitude);
}
...
void MAX30105_setProximityThreshold(uint8_t threshMSB) {
    ...
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_PROXINTTHRESH, threshMSB);
}
...
void MAX30105_disableSlots(void) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_MULTILEDCONFIG1, 0);
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_MULTILEDCONFIG2, 0);
}
...
void MAX30105_clearFIFO(void) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_FIFOWRITEPTR, 0);
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_FIFOOVERFLOW, 0);
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_FIFOREADPTR, 0);
}
...
uint8_t MAX30105_getWritePointer(void) {
    return (i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_FIFOWRITEPTR));
}

//Read the FIFO Read Pointer

uint8_t MAX30105_getReadPointer(void) {
    return (i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_FIFOREADPTR));
}
...
float MAX30105_readTemperature() {
    ...
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_DIETEMPCONFIG, 0x01);
    ...
    //while (TIME_getDiffTime(TIME_getCurrent(), startTime)*1000 < 100) {
    while ((millis() - startTime) < 100) {
        //uint8_t response = i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_DIETEMPCONFIG); //Original way
        //if ((response & 0x01) == 0) break; //We're done!

        //Check to see if DIE_TEMP_RDY interrupt is set
        uint8_t response = i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_INTSTAT2);
    ...
    int8_t tempInt = i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_DIETEMPINT);
    uint8_t tempFrac = i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_DIETEMPFRAC); //Causes the clearing of the DIE_TEMP_RDY interrupt
...
void MAX30105_setPROXINTTHRESH(uint8_t val) {
    i2c_write1ByteRegister(MAX30105_ADDRESS, MAX30105_PROXINTTHRESH, val);
}
...
uint8_t MAX30105_readPartID() {
    return i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_PARTID);
}

void MAX30105_readRevisionID() {
    MAX30105_revisionID = i2c_read1ByteRegister(MAX30105_ADDRESS, MAX30105_REVISIONID);
}
...
uint8_t MAX30105_available(void) {
    int8_t numberOfSamples = MAX30105_sense.head - MAX30105_sense.tail;
...
uint32_t MAX30105_getRed(void) {
    ...
    if (MAX30105_safeCheck(250))
        return (MAX30105_sense.red[MAX30105_sense.head]);
    else
...
uint32_t MAX30105_getIR(void) {
    //Check the sensor for new data for 250ms
    if (MAX30105_safeCheck(250))
        return (MAX30105_sense.IR[MAX30105_sense.head]);
    else
        return (0); //Sensor failed to find new data
}
...
uint32_t MAX30105_getGreen(void) {
    //Check the sensor for new data for 250ms
    if (MAX30105_safeCheck(250))
        return (MAX30105_sense.green[MAX30105_sense.head]);
    else
        return (0); //Sensor failed to find new data
}
...
uint32_t MAX30105_getFIFORed(void) {
    return (MAX30105_sense.red[MAX30105_sense.tail]);
}

//Report the next IR value in the FIFO

uint32_t MAX30105_getFIFOIR(void) {
    return (MAX30105_sense.IR[MAX30105_sense.tail]);
}

//Report the next Green value in the FIFO

uint32_t MAX30105_getFIFOGreen(void) {
    return (MAX30105_sense.green[MAX30105_sense.tail]);
}
...
void MAX30105_nextSample(void) {
    ...
    {
        MAX30105_sense.tail++;
        MAX30105_sense.tail %= STORAGE_SIZE; //Wrap condition
    }
}
...
//uint16_t MAX30105_check_0(void) {
...
//        //printf("numberOfSamples = %d, bytesLeftToRead = %d\n", numberOfSamples, bytesLeftToRead);
...
//            uint8_t data[toGet];
//            i2c_readDataBlock(MAX30105_ADDRESS, MAX30105_FIFODATA, data, toGet);
//            uint16_t i = 0;
//            while (toGet > 0) {
//                MAX30105_sense.head++; //Advance the head of the storage struct
//                MAX30105_sense.head %= STORAGE_SIZE; //Wrap condition
//
...
//
//                MAX30105_sense.red[MAX30105_sense.head] = tempLong; //Store this reading into the MAX30105_sense array
//
...
//                    tempLong &= 0x3FFFF; //Zero out all but 18 bits
//
//                    MAX30105_sense.IR[MAX30105_sense.head] = tempLong;
//                }
//
...
//                    tempLong &= 0x3FFFF; //Zero out all but 18 bits
//
//                    MAX30105_sense.green[MAX30105_sense.head] = tempLong;
//                }
//
...
uint16_t MAX30105_check(void) {
    ...
    if (readPointer != writePointer) {
        ...
        if (bytesLeftToRead > 0) {
            ...
            i2c_readDataBlock(MAX30105_ADDRESS, MAX30105_FIFODATA, data, bytesLeftToRead);
            int toGet = bytesLeftToRead;
            int i = 0;
            while (toGet > 0) {
                MAX30105_sense.head++; //Advance the head of the storage struct
...
bool MAX30105_safeCheck(uint8_t maxTimeToCheck) {
    //uint32_t markTime = TIME_getCurrent();
    uint32_t markTime = millis();
    while (1) {
        //uint32_t timeNow = TIME_getCurrent();
        //if (TIME_getDiffTime(timeNow, markTime)*1000 > maxTimeToCheck) return (false);
